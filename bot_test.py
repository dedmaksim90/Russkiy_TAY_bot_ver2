...
CATEGORIES = {
    ...
    "🍗 Мясо": {
        ...
        # Новые средние веса для частей
        "parts_weight": {
            "🍗 Грудка": 0.4,  # 1 упаковка ~400гр
            "🍗 Окорочка": 1.2,  # 3 шт в упаковке ~1.2кг
            "🍗 Крылья": 0.8,  # 8 шт в упаковке ~800гр
            "🍗 Спинки": 0.5,  # 1 упаковка ~500гр
            "❤️ Сердце": 0.5,  # 0.5 кг упаковка
            "🫔 Печень": 0.5,  # 0.5 кг упаковка
            "🦴 Шея": 1.0  # 1 шт ~1кг
        },
        # Упаковки для частей
        "parts_packaging": {
...
# ==================== БАЗА ДАННЫХ ====================
# Все данные теперь хранятся в SQLite (database.py)
# Глобальные переменные оставлены для совместимости, но используют DB

products_db = {}  # Кэш товаров (загружается при старте)
individual_products_db = {}  # Кэш индивидуальных товаров
orders_db = {}  # Кэш заказов
user_carts = {}  # Кэш корзин (в памяти)
notifications_db = {}  # Кэш уведомлений
product_views_db = {}  # Кэш просмотров
order_return_items_db = {}  # Кэш возвратов
manual_add_requests_db = {}  # Кэш ручных добавлений
user_stats_db = {}  # Кэш статистики пользователей
...
def add_individual_product(product_data: dict):
    ...
    return db.add_individual_product(product_data)
...
def add_order(order_data: dict):
    ...
    for item in order_data.get('items', []):
        db.add_order_item(
            ...
            item['quantity'],
            item['price'],
            item.get('total', item['price'] * item['quantity'])
        )
    return True
...
def update_order(order_id: str, update_data: dict):
    ...
    if 'status' in update_data:
        return db.update_order_status(order_id, update_data['status'])
    return db.update_product(order_id, update_data)

def add_admin(user_id: int):
    """Добавить админа"""
    return db.set_admin_status(user_id, True)

def remove_admin(user_id: int):
    """Удалить админа"""
    return db.set_admin_status(user_id, False)
...
def get_reviews(product_id: str):
    """Получить отзывы"""
    return db.get_product_reviews(product_id)
...
def get_rubrics_keyboard(category_name: str, subcategory_type: str, is_admin=False):
    ...
    subcategories = category.get("subcategories", {})
    rubrics_data = subcategories.get(subcategory_type, [])

    keyboard = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    
    # Проверяем, является ли rubrics_data словарем (новая структура с группами)
    if isinstance(rubrics_data, dict):
        # Новая структура: {"Тушки": [...], "Частями": [...], ...}
        for rubric_group, rubrics_list in rubrics_data.items():
            count = count_products_in_rubric_group(category_name, subcategory_type, rubric_group, rubrics_list)
            keyboard.add(KeyboardButton(f"{rubric_group} ({count})"))
    else:
        # Старая структура: [...]
...
def count_products_in_rubric_group(category_name: str, subcategory_type: str, rubric_group: str, rubrics_list: list) -> int:
    """Подсчет товаров в группе рубрик (например, все Тушки)"""
    count = 0
    for rubric in rubrics_list:
        for product in products_db.values():
            if (product.get('category') == category_name and
                product.get('subcategory_type') == subcategory_type and
                product.get('subcategory') == rubric and
                product.get('quantity', 0) > 0):
                count += 1
    return count

@dp.message_handler(lambda m: m.text in ["❄️ Охлаж������енное", "🧊 Замороженное"] or m.text.startswith("❄️ Охлажденное") or m.text.startswith("🧊 Замороженное"))
...
async def show_meat_rubric_group(message: types.Message):
    """Обработчик для выбора группы товаров в мясе - показывает список товаров"""
    # Извлекаем название группы (убираем счетчик)
    rubric_text = message.text.split(' (')[0]
    
    # Определяем группу товаров
    if rubric_text == "Тушки":
        rubric_group = "Тушки"
        items = ["🐓 Цыпленок бройлер", "🐔 Молодой петушок", "👑 Цесарка", "🐦 Перепелка"]
    elif rubric_text == "🥩 Частями":
        rubric_group = "🥩 Частями"
        items = ["🍗 Грудка", "🍗 Окорочка", "🍗 Крылья", "🍗 Спинки"]
    elif rubric_text == "🥣 Потроха":
        rubric_group = "🥣 Потроха"
        items = ["❤️ Сердце", "🫔 Печень", "🦴 Шея"]
    elif rubric_text == "🥩 Фарш":
        rubric_group = "🥩 Фарш"
        items = ["Фарш 60/40", "Фарш 100% грудка"]
    else:
        return
    
    # Формируем текст со списком
    text = f"📦 <b>{rubric_group}</b>\n\nВыберите товар:\n\n"
...
@dp.message_handler(text="↩️ Назад")
async def go_back_from_rubric(message: types.Message):
    """Возврат к выбору типа хранения"""
    user_is_admin = is_admin(message.from_user.id)
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    keyboard.add(
        KeyboardButton("❄️ Охлажденное"),
        KeyboardButton("🧊 Замороженное")
    )
    keyboard.add(KeyboardButton("↩️ К категориям"))
    await message.answer("🍗 М����со\n\nВыберите тип хранения:", reply_markup=keyboard)
...
async def on_startup(dp):
    ...
    print("   • 🗄️ Локальное хранение данных (SQLite)")
    print("=" * 50)
